...
if(root){
  ...
  async function setStep(i,id,instant=false){if(id!==runId)return;step=i;setCaption(i);const s=scenarios[scenarioIndex],d=instant?0:base;if(i===0){focus("consumer");side("consumer");els.request.classList.add("show");return}if(i===1){focus("consumer");show(els.ai);$("#demo-ai-1")?.classList.add("on");els.aiStatus.textContent="entendendo a situação…";await wait(d*.30,id);$("#demo-ai-2")?.classList.add("on");els.aiStatus.textContent="separando restrições…";await wait(d*.30,id);$("#demo-ai-3")?.classList.add("on");els.aiStatus.textContent="organizando a intenção…";show(els.parsed);els.tags.innerHTML="";for(const tag of s.tags){const span=document.createElement("span");span.textContent=tag;els.tags.appendChild(span);if(!instant)await wait(72,id)}return}if(i===2){focus("consumer");show(els.ai,false);show(els.assistant);await typeText(s.assistant,id,instant);return}if(i===3){focus("consumer");els.solutions.innerHTML=s.solutions.map(([name,detail,reason])=>`<div class="demo-solution"><div><small>CAMINHO DEMONSTRATIVO</small><strong>${name}</strong><span>${detail}</span></div><em>${reason}</em></div>`).join("");show(els.solutions);return}if(i===4){focus("bridge");side("company");els.bridge.classList.add("active");els.packet.classList.remove("travel");void els.packet.offsetWidth;els.packet.classList.add("travel");await wait(d*.48,id);show(els.opportunity);focus("company");els.open.textContent="1";els.newCount.textContent="1";els.bridge.classList.remove("active");return}if(i===5){focus("company");side("company");show(els.companyChat);els.n1.classList.add("show");await wait(d*.34,id);focus("consumer");side("consumer");els.companyQuestion.classList.add("show");return}if(i===6){focus("consumer");side("consumer");els.answer.classList.add("show");await wait(d*.32,id);focus("company");side("company");els.n2.classList.add("show");await wait(d*.24,id);els.n3.classList.add("show");return}if(i===7){focus("company");side("company");show(els.quote);await wait(d*.30,id);focus("consumer");side("consumer");els.consumerQuote.classList.add("show");return}if(i===8){focus("consumer");side("consumer");els.accept.classList.add("show");await wait(d*.30,id);focus("company");side("company");show(els.quoteState);els.open.textContent="0";await wait(d*.26,id);focus("consumer");side("consumer");els.done.classList.add("show");return}if(i===9){focus("bridge");side("company");els.bridge.classList.add("active");await wait(d*.44,id);els.bridge.classList.remove("active");return}}
  async function autoplay(){const id=++runId;while(id===runId){resetVisual();applyScenario();for(let i=0;i<captions.length;i++){if(id!==runId)return;await setStep(i,id);if(i<captions.length-1 && !(await wait(stepHold(i,scenarios[scenarioIndex]),id)))return}if(!(await wait(1100,id)))return;scenarioIndex=(scenarioIndex+1)%scenarios.length}}
