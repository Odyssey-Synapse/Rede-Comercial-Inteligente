...
export default async function handler(req, res) {
  ...
  const payload = {
    ...
    html: `<div style="font-family:Arial,sans-serif;line-height:1.5;color:#263D34"><h2>Nova mensagem pelo site do Uai Perto</h2><p><strong>Nome:</strong> ${esc(name)}<br><strong>E-mail:</strong> ${esc(email)}<br><strong>Assunto:</strong> ${esc(subject)}</p><hr><p style="white-space:pre-wrap">${esc(message)}</p><hr><p style="font-size:12px;color:#52665E">Enviado pelo formulário oficial do Uai Perto.</p></div>`
  };
