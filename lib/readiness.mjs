...
export function productionReadiness(env = process.env) {
  const provider = String(env.CNPJ_PROVIDER || "cnpjws").trim().toLowerCase();
  const providerSupported = ["cnpjws", "serpro"].includes(provider);

  const checks = [
    ["CNPJ_PROVIDER", providerSupported, "Provedor de consulta CNPJ configurado"],
    ...
    ["RESEND_FROM_EMAIL", !!env.RESEND_FROM_EMAIL, "Remetente configurado no Resend"],
  ...
  return { ready: results.every(x => x.ok), provider, checks: results, blockers: results.filter(x => !x.ok) };
}
